...
//   model / Text Encoder / VAE / 输出前缀 → 标签左 | 控件右（两栏 grid）
//   LoRA 行                              → 标签 | 动态列表 | 「+ 添加」（三栏 grid）
//   出图张数 / 长边尺寸 / 默认比例        → 保持紧凑一行三列（.rs-gen-grid）
  ...
  <div class="rs-skill-modal rs-skill-detail" style="width:640px;">
    <div class="rs-skill-modal-content">
      <div class="rs-gen-settings rs-skill-gen-settings">
        <div class="rs-config-row"><label class="rs-form-label">🖼️ 出图设置（本技能覆盖）</label></div>
        <div class="rs-gen-model-section">
          <div class="rs-config-row"><label class="rs-form-label">Model</label><select></select></div>
          <div class="rs-config-row"><label class="rs-form-label">Text Encoder</label><select></select></div>
          <div class="rs-config-row"><label class="rs-form-label">VAE</label><select></select></div>
        ...
        <div class="rs-gen-size-section">
          <div class="rs-gen-grid">
            <div class="rs-config-row"><label class="rs-form-label">出图张数</label><input class="rs-form-input" type="number"></div>
          ...
          <div class="rs-config-row"><label class="rs-form-label">输出前缀</label><input class="rs-form-input" type="text"></div>
        </div>
      </div>
    </div>
  </div>
...
test("技能弹窗出图设置：model/encoder/VAE/前缀两栏，张数/长边/比例一行三列", async (t) => {
    let browser;
    try {
    ...
    try {
        ...
        const r = await page.evaluate(() => {
            ...
            return {
                ...
                prefix: colsOf(".rs-skill-gen-settings .rs-gen-size-section > .rs-config-row"),
                grid: colsOf(".rs-skill-gen-settings .rs-gen-grid"),
            };
        });

        assert.equal(r.model, 2, `Model/Encoder/VAE 行应为两栏（label | 控件），实际 ${r.model}`);
        assert.equal(r.lora, 3, `LoRA 行应为三栏（label | 列表 | 添加按钮），实际 ${r.lora}`);
        assert.equal(r.prefix, 2, `输出前缀行应为两栏，实际 ${r.prefix}`);
        assert.equal(r.grid, 3, `出图张数/长边尺寸/默认比例应保持一行三列，实际 ${r.grid}`);
    } finally {
