...
- **宫格图拆分**：「🎨 分镜故事板」页的「🧩 宫格图拆分」卡片——选一张带分隔条/留白的分镜宫格图，`POST /rs_recipes/grid_split` 用 `grid_split.py`（纯 PIL）做均匀间隙检测自动判行列（也可手动指定行×列，1~12；检不出时回退整幅一格或等分），按行优先顺序把各格裁到 `input/`（格子不带分隔条）。拆分即**替换现有分段**（旧段有内容时先确认）：每格成为该段的**首帧与分镜图**（i2v），对照表「分镜图」列逐格显示。点「✨ 逐格描述提示词」把各格图按顺序喂给多模态 LLM（`director_panel_describe` 任务，`POST /rs_recipes/director_describe_panels`），每格生成一条视频提示词写回对应段；结果数量与格子数不一致会拒绝。面板随 `first_frame` / `storyboard` 在保存时落进配方 `assets/`，重开回显。
