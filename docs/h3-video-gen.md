...
- **宫格图拆分**：「🎨 分镜故事板」页的「🧩 宫格图拆分」卡片——与「🎨 图片分镜」「统一图片」**三选一**（`story.frame_source` = `grid`，随配方落盘回显）。选一张带分隔条/留白的分镜宫格图，`POST /rs_recipes/grid_split` 用 `grid_split.py`（纯 PIL）做均匀间隙检测自动判行列（也可手动指定行×列，1~12；检不出时回退整幅一格或等分），按行优先顺序把各格裁到 `input/`（格子不带分隔条）。拆分即**替换现有分段**（旧段有内容时先确认）并自动切到 `grid` 方式：每格成为该段的**首帧与分镜图**（i2v），对照表「分镜图」列逐格显示。点「✨ 逐格描述提示词」由前端**按格子自动循环**调用单格端点 `/rs_recipes/director_describe_panel`（`director_panel_describe` 任务，一次只喂一张分镜图作该段首帧 + 时长），每格单独生成一条可直接提交的 **MiniMax H3 i2v 成品提示词**写回对应段；逐格反馈「第 i/N 格」进度、单格失败不中断其余。面板随 `first_frame` / `storyboard` 在保存时落进配方 `assets/`，重开回显。
