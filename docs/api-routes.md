...
| POST | `/rs_recipes/director_generate_segments` | 导演编辑器「📖 故事分镜」文字故事板：主题（`idea`）或已写好的故事脚本（`script`）+ 分段粒度（`segment_seconds`）→ LLM（任务 `director_story`）**直接输出分段 JSON**，返回 `segments[]`（每段 `prompt` / `duration_sec` / `storyboard_prompt`）；角色参考图文件名列表（`characters`）以多模态附上锁身份 |
