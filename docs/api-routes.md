...
| POST | `/rs_recipes/grid_split` | 导演编辑器「🧩 宫格分镜图拆分」：一张带分隔条/留白的分镜宫格图 → 纯 PIL 均匀间隙检测（或手动行×列，1~12）自动判行列，按行优先顺序把各格裁到 `input/`，返回 `{rows, cols, panels:[{filename,width,height,preview_url}]}`（格子数超上限拒绝） |
