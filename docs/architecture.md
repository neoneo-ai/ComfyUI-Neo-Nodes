...
| `skill.py` | 技能系统：Markdown + YAML frontmatter 解析（PyYAML 事件流）、`skills/{presets,tasks,custom}/<id>/skill.md` 扫描与加载（`scan_skills` / `load_skill_content` / `load_task_template`）、多结果契约读取、语言互斥主文件选择（`SKILL.md`/`SKILL.cn.md`）与按需引用加载的工具调用代理循环（`run_skill_agent[_stream]` / `read_skill_file`，仅运行时惰性导入 llm 原语以避免与 llm.py 的顶层依赖形成循环）、`audit: h3` 技能的生成后审计接入（`_h3_audit_and_repair` / `_h3_audit_events`，流式路径以 status/replace 事件上报自检与修复阶段）、工作流上下文按模式注入的最终 grounding 约束（`_h3_grounding_check`）、`/rs_prompts/skill*` 路由（列表/读取/保存/删除/上传） |
| `prompt_audit.py` | H3 提示词确定性格式审计（纯规则、零 token）：六段/三字段结构顺序、时间戳格式与时长上限、参考标签与工作流连线一致性、对白说话人 ID、内部表示术语泄漏；并提供窄修复消息构造（`narrow_repair_messages`）与修复验收（`repair_acceptable`），供 skill.py 在 `audit: h3` 技能生成后调用 |
