...
| `director.js` | 多段视频导演编辑器（配方编辑窗口，从 recipes.js 拆出）：shared 分辨率（宽高比/百万像素或自定义 W/H）+ 逐段 skill/提示词/首帧/时长 + 半自动故事生成与拆分；时间轴复用 `director-timeline.js`，保存走 `recipes.js` 的 `saveRecipe` |
