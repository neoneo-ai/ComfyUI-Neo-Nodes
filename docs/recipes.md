...
- **半自动故事生成（「📖 文字故事板」页签）**：
  ...
  - 故事板页**左右两栏**：左栏是**分段前故事**（填主题/想法，点 **✨ 自动生成故事**，后端 `/rs_recipes/director_generate_story` 用 LLM 产出完整故事脚本、可编辑），右栏是**分段后的故事**、其标题行最右侧是 **分段粒度**（5 / 10 / 15 秒）与 **✅ 确认并拆分到时间轴**。角色/背景参考图不再随「生成故事」「拆分」发给 LLM——各段角色/场景一致性改由「🎨 分镜故事板」页 **🎨 图片分镜**卡片的 r2i 参考编辑保证（见下条）。点 **✅ 确认并拆分到时间轴**，后端 `/rs_recipes/director_split_segments` 把故事切成约目标秒数的若干场景，前端据此**替换**时间轴现有段落（技能取首个可用视频技能、每段 `mode` 为 `t2v`），并把各段（序号 + 时长 + 提示词）**填入右栏**供左右对照，随后编辑器**自动切到「🎨 分镜故事板」页**做全局准备（分镜方式 / 参考素材 / 提示词优化；生成模式在时间轴页选择），需要逐段微调时再切到「🎞️ 时间轴分段」页。**打开旧配方时右栏回显优化前原文**（优先用落盘的 `setup.orig_prompts` 快照，未存过则用段内当前提示词；优化结果不覆盖右栏）。
  - **故事板内容随配方保存**：主题 / 故事脚本 / 分段粒度 / 角色·背景参考图在保存时一并写入 `recipe.json`（归档于可选字段 `story`），重新打开该配方时原样回显；全新配方且未用过故事板时不写入该字段。
- **图片分镜（关键帧）**：「🎨 分镜故事板」页的 **🎨 图片分镜**卡片（统一参考区之后、提示词优化之前；t2v/i2v/fl2v 且分镜方式为逐段图片分镜时显示）：**生图模式**（radio 单选）t2i 文生图 / r2i 参考编辑 + **生图技能**（仅列生图类技能；t2i 默认 Krea2 文生图、r2i 默认 Qwen Image 2.1，各模式分别记住上次选择），**🎨 生成图片分镜**（与状态一起靠「生图模式 / 生图技能」行最右，不独占一行）由后端 `/neo_video_gen/storyboard_generate`（请求带 `mode`）逐段串行出关键帧——同一次生成各段**共用同一 seed**（提示词仍逐段不同），**再次点击强制重新生成全部段**（换新随机种子，不重复旧图），落配方自己的 `assets/storyboard_{recipe}_{nn}.png`（缩略图走 `/rs_recipes/asset?recipe=…&file=…`，不经 `input/`；旧配方需重新生成分镜才有缩略图）。**t2i**：纯文生图，不挂任何参考图；**r2i**：卡片内显示**角色/背景参考图**两组「已用列表」网格（点击黑色空区上传或从素材库拖入、拖放排序，角色 ≤6 / 背景 ≤4，合计 ≤10），第 1 张为编辑目标；两种模式都按所选技能执行、不再强制切 Qwen Image 2.1（仅旧 API 不带 `mode` 时保留「带参考图的段固定 Qwen」的旧行为，该段留 warning）。生成的分镜对 i2v/fl2v 段自动用作首帧（受下方「分镜 / 首帧方式」控制），并即时回显到本页对照表第一列（时间轴段行不再重复展示缩略图）。**分镜图用的提示词是拆分时产出的 `storyboard_prompt`（必填，静态画面 / 该段起始画面 / 不写镜头运动），不是视频段提示词**；旧配方或未带该字段的分镜快照会回退视频提示词并在该段 `warnings` 里点名，前端完成文案与提示条会列出这些段号——视频提示词写的是运动过程，不一定适合生图，建议重新拆分或补上分镜提示词。**角色参考图同时是视频各段的身份参考**（见 [h3-video-gen.md](h3-video-gen.md) 的身份参考）：分镜关键帧是背影 / 局部特写、看不到脸时，靠它在生成视频时锚住角色身份；本页的**角色身份参考**复选框（默认开）可关掉它做前后对比测试，取消勾选后保存写 `shared.identity_refs=false`。
 - **分镜 / 首帧方式**：「🎨 分镜故事板」页 radio 单选，二选一——**逐段图片分镜**（默认：生成的分镜自动回填 i2v/fl2v 各段首帧）↔ **统一图片**（i2v/fl2v 用统一首帧网格；此时 🎨 卡片隐藏）。切回「逐段图片分镜」时把各段当前首帧**无损回填**为 `first_frame`，再走分镜自动回填逻辑。
