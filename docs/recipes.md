...
- **图片分镜（关键帧）**：「🎨 分镜故事板」页的 **🎨 图片分镜**卡片（统一参考区之后、提示词优化之前；t2v/i2v/fl2v 且分镜方式为逐段图片分镜时显示）：**生图模式**（radio 单选）t2i 文生图 / r2i 参考编辑 + **生图技能**（仅列生图类技能；t2i 默认 Krea2 文生图、r2i 默认 Qwen Image 2.1，各模式分别记住上次选择），**🎨 生成图片分镜**（与状态一起靠「生图模式 / 生图技能」行最右，不独占一行）由后端 `/neo_video_gen/storyboard_generate`（请求带 `mode`）逐段串行出关键帧，落 `input/NeoDirector/storyboard_{recipe}_{nn}.png`。**t2i**：纯文生图，不挂任何参考图、链式参考无效；**r2i**：卡片内显示 **链式参考**（默认开，用最近生成的分镜作下一段参考）与**角色/背景参考图**两组「已用列表」网格（点击黑色空区上传或从素材库拖入、拖放排序，角色 ≤6 / 背景 ≤4，合计 ≤10），第 1 张为编辑目标；两种模式都按所选技能执行、不再强制切 Qwen Image 2.1（仅旧 API 不带 `mode` 时保留「带参考图的段固定 Qwen」的旧行为，该段留 warning）。生成的分镜对 i2v/fl2v 段自动用作首帧（受下方「分镜 / 首帧方式」控制），并即时回显到本页对照表第一列（时间轴段行不再重复展示缩略图）。
