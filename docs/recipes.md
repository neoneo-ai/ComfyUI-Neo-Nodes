...
- **「🎯 统一设置」页签（拆分后的中间步骤）**：拆分成功后编辑器自动切到本页，先做全局准备、再到时间轴逐段微调：
  ...
  - **统一素材区（按生成模式显示，与每段的素材要求一致）**：**全参考 r2v** → 图 / 视频 / 音频三组多选网格（与每段同一组件、同样上限 ≤9 / ≤3 / ≤3，支持本地上传与素材库拖入）；**图生 i2v** → 统一首帧单选网格；**首尾帧 fl2v** → 统一首帧 + 尾帧网格；**文生 t2v / 混合 mixed** → 显示说明文字（文生无需素材；混合模式各段素材要求不同，需到时间轴页逐段设置）。点 **应用到所有分段** 把当前所选**覆盖式**写入所有分段的对应区域（参考区或首帧/尾帧）；各段已有的「同步到所有分段」行为保留，供后续逐段微调。
