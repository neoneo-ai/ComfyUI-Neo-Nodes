...
- **顶栏「修复工作流」按钮**：对**当前画布**工作流一键检查并修复。检测到失效路径时弹出二次确认框（原路径红色删除线 → 候选新路径绿色），选择「修复并载入」后画布原地更新（节点位置保留）；接口不可用时给出警告且画布保持原样；无失效路径时提示「检查完成」。
- **载入前自动检查**：从素材灯箱「⤷ 工作流」或配方「📋 复制工作流」载入时，同样先走修复检查 + 二次确认，修复接口不可用会以警告提示「已按原样载入」。
...
├── workflow.py             # 工作流后端：模型路径修复逻辑 + 修复 API（/neo_nodes/repair）
...
└── web/                    # 前端资源
    ...
    ├── recipes.css         # 配方样式
    ├── workflow.js         # 工作流功能（模型路径修复请求+二次确认+日志 + 顶栏「修复工作流」/「修复记录」按钮）
