...
2. **配置 LLM（二选一）**
   ...
   - **本地**：把 GGUF 模型放入 `models/LLM/`，Settings → Provider 选「Local GGUF」并选择模型后点 💾 保存（目录只有一个模型时可跳过，运行时自动使用）。
...
- **Skill系统** - 通过技能选择器调用不同能力，按组分类：生图、图像提示词增强、视频提示词增强（*generator / H3 文生视频 / 图生视频 / 全能参考）、图像/反推、任务（含中英互译等指令）、自定义
...
一个技能是一个或多个 Markdown 文件（主文件 `skill.md`），头部为 YAML frontmatter（含 `name`、`tags`、`max_tokens`、`multi_turn` 等字段），正文即 System Prompt 内容：
