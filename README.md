...
在 **Neo Prompt** 节点上点击 💾 保存按钮，弹窗底部直接提供「保存提示词」与「🧊 保存配方」两个确认按钮（配方按钮按节点启用），命名确认后配方会收集**当前节点所在连通子图**内已连线的 `LoadImage` / `LoadVideo` / `LoadAudio` 资源与节点上的提示词，保存为一个配方。弹窗内实时显示将收集的资源数量；弹窗以固定定位浮层显示，不再受节点窗口大小截断。
...
- **自动对齐**：资产数与该子图启用的连线加载节点数不一致，且同类节点（启用 + 禁用）全部连到同一个下游目标节点时，自动启用 Bypass/Never 节点补齐，或把多余的启用节点设为 Bypass（保留参数位靠前的参与还原）
