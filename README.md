...
> **多轮交互技能**：文本区下方常驻一行提示——每次生成仅推进一个阶段，需把返回的问询补充完整后再次点击 ✨ 继续下一阶段。

### 模板与技能管理

点击节点底部 ⚙️ 打开设置窗口，「📝 Prompt Templates」标签页提供完整的技能管理功能：

- **新建技能** - 点击 "+ New Skill"，填写名称与 System Prompt 内容后保存
- **上传导入** - 点击 "⬆ ZIP" 或 "⬆ Folder"，将 `.zip` 包或整个文件夹（含多个 `.md`）导入为新技能
- **搜索过滤** - 按名称实时过滤技能列表
- **查看 / 编辑** - 点击列表条目或 👁 图标加载技能，默认以 Markdown 渲染预览正文；点「✎ Edit」切换到原始文本编辑、「👁 Preview」切回预览（新建技能直接进入编辑模式）
- **多文件** - 一个技能可含多个 `.md` 文件，正文按文件名升序拼接；正文区提供下拉列表切换查看/编辑各个 `.md`（主文件 `skill.md` 只编辑其正文、保留 frontmatter 元数据），可用「+ File」新增、「🗑」删除当前选中的附属文件
- **复制为副本** - 内置模板与任务技能不可删除，可一键复制为自定义副本后自由修改
- **删除** - 仅用户自定义（USR）技能可删除，内置（SYS / TASK）受保护
- **自动同步** - 保存、上传、复制、删除后广播更新事件，所有节点上的技能选择器立即刷新

一个技能是一个或多个 Markdown 文件（主文件 `skill.md`），头部为 YAML frontmatter（含 `name`、`tags`、`max_tokens` 等字段），正文即 System Prompt 内容：
