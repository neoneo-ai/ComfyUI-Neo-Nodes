...
│   └── <配方名>/
│       ├── recipe.json          # 元数据：name / prompt / created_at / assets / kinds
...
在 **Neo Prompt** 节点上点击 💾 保存按钮，弹窗顶部选择「🍱 配方」模式（默认「📝 提示词」为普通预设保存），命名确认后会收集**当前节点所在连通子图**内已连线的 `LoadImage` / `LoadVideo` / `LoadAudio` 资源与节点上的提示词，保存为一个配方。配方模式下跳过 AI 标题/标签分析，弹窗内会实时显示收集到的资源数量；弹窗以固定定位浮层显示，不再受节点窗口大小截断。
...
配方始终保存到 `recipes/custom/`；与内置预设同名的保存会被拒绝。侧边栏配方页按「我的配方 / 内置预设」分组展示，🗑 仅可删除自定义配方。旧版直接放在 `recipes/` 下的配方会在启动时自动迁移到 `custom/`。节点上的预设列表同样并入配方条目，与提示词统一按修改时间降序排序，以立方体图标区分，点击回填提示词并还原资源到所在子图，自定义配方可在列表中直接删除。点击卡片缩略图或标题可打开详情浮层，查看完整标题、完整提示词与全部资源（视频可直接预览、音频可试听），浮层内也可直接发送到工作流。
