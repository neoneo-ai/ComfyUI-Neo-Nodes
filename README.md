...
| ⚡ Neo Prompt Agent | 节点 | 提示词管理 + AI 生成：内置大量预设、可搜索快速筛选；输出 PROMPT 文本 + BUNDLE 运行时包（BUNDLE 可直连 🎨 Krea2 / 🎞️ H3 Director，一次性带上 prompt/参考图（连线·@引用·本地上传）/skill），无需连 CLIP / 文本编码器 | [prompts](docs/prompts.md) |
...
| 🎞️ H3 Video Director | 节点 | MiniMax H3 视频生成入口：以 `video_director` 配方为参数逐段生成（每段自带 skill/提示词/时长/首帧/尾帧，模式可选 文生 / 图生 / 首尾帧 / 全参考 / 混合）并拼接成单个含原生音频 `VIDEO`；图生段携带首帧、首尾帧段额外携带尾帧（可只给一边）、全参考段携带 ≤9 图 / ≤3 视频 / ≤3 音频（未挂槽位自动裁掉）；Tier A 连续性（上段尾帧→下个图生/首尾帧段首帧、丢边界重复帧、音频按帧对齐）。也可连 ⚡ Neo Prompt Agent 的 **BUNDLE** 输入按单片段生成（skill/提示词/参考图取自 bundle，忽略配方）。编辑器另支持**半自动故事生成**：LLM 由主题产出故事脚本，确认后按 5/10/15s 拆分场景并结合角色/背景参考图重生成每段提示词填充时间轴，随后自动进入「🎯 统一设置」页（全局模式 / 按模式的统一素材一键应用到所有分段 / LLM 按 H3 官方格式批量重写各段提示词），再到时间轴逐段微调；每步状态（故事板 / 拆分后的分段 / 统一设置区所选素材）随配方保存，重新打开编辑器原样回显、可继续编辑；关闭时若有未保存修改会先弹确认条（💾 保存并关闭 / 放弃修改 / 继续编辑），无修改直接关；节点内时间轴左侧「👁」开关控制采样期间实时预览（taeh3 真彩多帧动画面板：自动循环 + 暂停/逐帧/逐步回看，缺文件回落 Latent2RGB，与全局预览设置无关） | [h3-video](docs/h3-video-gen.md) |
